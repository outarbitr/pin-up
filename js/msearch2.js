    ...
    load: function (params, callback, append) {
        ...
        $.post(mse2Config.actionUrl, params, function (response) {
            ...
            if (response.success) {
                ...
                $('#mse2_filters fieldset.formatting').each(function() {
                    var container = $(this);
                    var formatting_filters_array = [];
                    var formatting_filters_chunks = [];
                    
                    formatting_filters_array.length = 0;
                    formatting_filters_chunks.length = 0;
                    
                    $(this).find('label').each(function() {
                        //Перестраиваем только видимые фильтры
                        if (!$(this).hasClass('disabled')) {
                            var formatting_filter_value = $(this).find('input').val();
                            var formatting_filter_total = Number($(this).find('sup').text());
                            
                            formatting_filters_array[formatting_filter_value] = formatting_filter_total;
                            formatting_filters_chunks[formatting_filter_value] = $(this);
                        }
                    });
                    
                    var formatting_filters_array_sorted = mSearch2.getSortedKeys(formatting_filters_array);
                    
                    //Рекурсировать массив
                    console.log(formatting_filters_array_sorted);
                    console.log(formatting_filters_chunks);

                    for (var key in formatting_filters_array_sorted){
                        var value = formatting_filters_array_sorted[key];
                        var item = formatting_filters_chunks[value];
